...
-export([
    % exprs/2,exprs/3,exprs/4,expr/2,expr/3,expr/4,
        translate/1,
        expr/5
    % ,
...
-spec(translate(Expression) ->
             EvaldExpression when
      Expression :: expression(),
      EvaldExpression :: evaldexpression()).
translate({Atom, Number, ArgList} = _Datum) when is_list(ArgList) ->
    {{Atom, Number, lists:map(fun translate/1, ArgList)}, uneval};
translate({Atom, Number, Op, Arg1, Arg2}) when is_tuple(Arg1) and is_tuple(Arg2) ->
    {{Atom, Number, Op, translate(Arg1), translate(Arg2)}, uneval};
translate({Atom, Number, Arg1, Arg2}) ->
    {{Atom, Number, translate(Arg1), translate(Arg2)}, uneval};
...
%     % HERE HERE HERE
%     'Elixir.IO':inspect({expr_grp, Fs, Bs0, EvalFun}),
%     Res = eval_bits:expr_grp(Fs, Bs0, EvalFun),
%     case Res of
%         {value,V,Bs} -> ret_expr(V, Bs, RBs)
%     end;
...
expr_list([E|Es], Vs, BsOrig, Bs0, Lf, Ef) ->
    ...
    case V of 
        %%% OMG money
        suspend -> 
            % 'Elixir.IO':inspect({expr_list_suspension_suspended, V, BsOrig}),
            {suspension, V, BsOrig, Bs1};
        % , RBs);
